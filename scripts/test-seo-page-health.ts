...
async function main() {
    ...
    const fetchStub = createFetchStub({
        ...
        [`${siteUrl}/cjk-ok`]: {
            ok: true,
            body: '<html><head><title>這是一個足夠長而且明確超過二十字的中文標題用來測試</title><meta name="description" content="這是一段中文 description，長度超過五十字，應該被視為可接受，不需要再被日報當成太短警告。"><meta property="og:title" content="og" /><script type="application/ld+json">{}</script></head></html>',
        },
        [`${siteUrl}/bad`]: {
            ok: true,
    ...
    const cjkGoodPage = await inspectPageMetadata(siteUrl, '/cjk-ok', 'CJK OK', fetchStub);
    assert.equal(cjkGoodPage.issues.length, 0);
