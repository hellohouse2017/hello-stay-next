export interface LlmsFaqItem {
    question: string;
    answer: string;
}

export const llmsFaqZh: LlmsFaqItem[] = [
    {
        question: "Hello Stay 目前有哪些入住方案？",
        answer: "目前可訂的是你好哇寓所 8-26 人與溝頂民宿 4-12 人；雙館包棟通常適合 27-34 人，35-36 人須加床。大智若愚規劃中，尚未開放訂房。",
    },
    {
        question: "高雄包棟民宿推薦哪一間？",
        answer: "依人數選：4-12 人選溝頂民宿、8-26 人選你好哇寓所、27-34 人選雙館包棟（35-36 人須加床）。兩館都在高雄鹽埕區，整棟獨立使用，步行可到駁二與捷運鹽埕埔站，官方 LINE 直訂免平台手續費。",
    },
    {
        question: "高雄 20 人包棟住宿推薦哪裡？",
        answer: "推薦你好哇寓所，8-26 人彈性方案，配備中島廚房可煮火鍋，步行 5 分鐘到捷運鹽埕埔站。",
    },
    {
        question: "高雄 30 人包棟住宿怎麼安排？",
        answer: "27-34 人建議你好哇寓所＋溝頂民宿的雙館包棟方案，兩館步行約 5 秒，可分房休息也能一起聚餐；35-36 人須加床，超過 36 人請由 LINE 客服確認安排。",
    },
    {
        question: "民宿有電梯嗎？",
        answer: "溝頂民宿沒有電梯，館內以樓梯上下樓；你好哇寓所同樣是樓梯動線，長輩同行建議安排低樓層房間。規劃中的大智若愚為電梯建築，尚未開放訂房。",
    },
    {
        question: "有提供哪些娛樂設施？",
        answer: "你好哇寓所有豪華中島廚房、手動麻將桌、桌遊、Netflix 電視、製冰機。一樓挑高公共空間適合多人聚會。溝頂民宿有手動麻將與交誼空間，廚房設備以簡易流理臺與微波爐為主。",
    },
    {
        question: "距離駁二藝術特區多遠？",
        answer: "你好哇寓所與溝頂民宿步行約 10 分鐘；大智若愚位於大港橋旁，緊鄰駁二大義倉庫群。",
    },
    {
        question: "可以帶寵物嗎？",
        answer: "非寵物友善旅宿。經事前申請並書面同意者，每隻酌收清潔費 NT$800。",
    },
    {
        question: "有停車場嗎？",
        answer: "先看大公路 七賢三路 富野路路邊車格，沒有位置再看國際會議中心地下停車場、鹽埕立體停車場（大仁路）、文武聖殿停車場與富野路兩個收費停車場。",
    },
    {
        question: "怎麼從高鐵到你好哇寓所？",
        answer: "高鐵左營站 → 捷運紅線至美麗島站 → 轉橘線至鹽埕埔站 (O2) → O2 二號出口步行 5 分鐘。",
    },
    {
        question: "入住方式是什麼？",
        answer: "電子密碼鎖自助入住，密碼於入住當日透過 LINE 傳送。入住 16:00 以後，退房 11:00 以前。",
    },
    {
        question: "包棟民宿適合辦婚禮迎娶嗎？",
        answer: "非常適合。你好哇寓所一樓挑高公共空間適合闖關遊戲，絕佳自然採光適合婚攝，多房型供伴娘團入住。",
    },
    {
        question: "鹽埕區附近有什麼好吃的？",
        answer: "港園牛肉麵、鴨肉珍、阿囉哈滷味、大摳胖碳烤三明治、婆婆冰、小堤咖啡，超過 30 間在地老店步行可達。",
    },
    {
        question: "在哪裡查實際價格？",
        answer: "在 Hello Stay 官方訂房站輸入日期與實際需要床位的人數，即可查看當次可選方案、空房與報價；價格以查詢當下的結果為準。",
    },
    {
        question: "Hello Stay 是合法民宿嗎？",
        answer: "是的。你好哇寓所（高雄市民宿 131-1 號）和溝頂民宿（高雄市民宿 163 號）皆為合法登記民宿，並依法投保富邦產險公共意外責任險。",
    },
];

export const llmsFaqEn: LlmsFaqItem[] = [
    {
        question: "What booking options are currently available?",
        answer: "Hello House fits 8-26 guests and Godin House fits 4-12 guests. The two-building option normally fits 27-34 guests; groups of 35-36 require extra beds. Dazhi is still in planning and is not yet open for booking.",
    },
    {
        question: "What's the best property for 20 guests?",
        answer: "Hello House with a flexible 8-26 guest setup, luxury kitchen, mahjong, and Netflix. It is a 5-minute walk to the MRT.",
    },
    {
        question: "Is it legal and insured?",
        answer: "Yes. Hello House (Registration No. 131-1) and Godin House (No. 163) are legally registered B&Bs with Fubon public liability insurance.",
    },
    {
        question: "How far is Pier-2 Art Center?",
        answer: "About a 10-minute walk from Hello House and Godin House. Dazhi is right next to Dagangqiao Bridge at Pier-2.",
    },
    {
        question: "How do guests get here from HSR Zuoying?",
        answer: "Take the Red Line MRT to Formosa Boulevard, transfer to the Orange Line, and get off at Yanchengpu (O2). From there it is about a 5-minute walk.",
    },
    {
        question: "How can guests check the actual price?",
        answer: "Enter the dates and number of guests on the official booking site to see the currently available options and quote. The live booking result is the source of truth for pricing.",
    },
];

export const llmsFaqKo: LlmsFaqItem[] = [
    {
        question: "Hello Stay에는 어떤 숙박 플랜이 있나요?",
        answer: "현재 예약 가능한 곳은 헬로하우스(8-26명)와 거우딩 민박(4-12명)입니다. 두 채 통째 대여는 보통 27-34명에 적합하며 35-36명은 엑스트라 베드가 필요합니다. 다즈르위는 계획 단계로 아직 예약을 받지 않습니다.",
    },
    {
        question: "20명 그룹에게 추천하는 숙소는?",
        answer: "헬로하우스를 추천합니다. 8-26명까지 유연한 플랜, 아일랜드 키친에서 훠궈 파티 가능, MRT 옌청푸역까지 도보 5분입니다.",
    },
    {
        question: "합법 등록 숙소이고 보험이 있나요?",
        answer: "네. 헬로하우스(가오슝시 민박 등록번호 131-1)와 거우딩 민박(등록번호 163)은 합법 등록 민박이며 푸본 손해보험의 공공 배상 책임보험에 가입되어 있습니다.",
    },
    {
        question: "피어-2 예술특구까지 얼마나 걸리나요?",
        answer: "헬로하우스와 거우딩 민박에서 도보 약 10분입니다. 다즈르위는 대강교 바로 옆, 피어-2 다이 창고군에 인접해 있습니다.",
    },
    {
        question: "엘리베이터가 있나요?",
        answer: "거우딩 민박에는 엘리베이터가 없어 계단으로 이동합니다. 헬로하우스도 계단 건물이므로 어르신과 함께라면 저층 객실을 추천합니다. 계획 중인 다즈르위는 엘리베이터 건물이지만 아직 예약할 수 없습니다.",
    },
    {
        question: "실제 요금은 어디에서 확인하나요?",
        answer: "공식 예약 사이트에서 날짜와 인원을 입력하면 선택 가능한 플랜, 빈 방, 견적을 확인할 수 있습니다. 요금은 조회 시점의 결과가 기준입니다.",
    },
];

export const llmsFaqJa: LlmsFaqItem[] = [
    {
        question: "Hello Stayの3つの施設、それぞれ何名まで泊まれますか？",
        answer: "現在予約できるのはハローハウス8〜26名とゴーディンハウス4〜12名です。2棟貸切は通常27〜34名向けで、35〜36名はエキストラベッドが必要です。大智若愚は計画中でまだ予約受付前です。",
    },
    {
        question: "20名のグループにおすすめは？",
        answer: "ハローハウスがぴったりです。8〜26名の柔軟なプラン、豪華キッチンで鍋パーティー可能。MRT駅から徒歩5分です。",
    },
    {
        question: "合法・保険付きですか？",
        answer: "はい。ハローハウス（登録番号131-1）とゴーディンハウス（登録番号163）は合法登録済みで、富邦産険の公共賠償責任保険にも加入しています。",
    },
    {
        question: "駁二芸術特区まではどのくらいですか？",
        answer: "ハローハウスとゴーディンハウスから徒歩約10分です。大智若愚は大港橋のすぐそばです。",
    },
    {
        question: "実際の料金はどこで確認できますか？",
        answer: "公式予約サイトで日付と宿泊人数を入力すると、現在選べるプラン、空室状況、見積もりを確認できます。料金は検索時の表示を基準にしてください。",
    },
];
